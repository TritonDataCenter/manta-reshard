...
phase_create_morays(ctl)
{
	...
	mod_vasync.waterfall([ function (done) {
		 ...
		 */
		status.update('listing instances of "moray" (%s)', p.shard);
		 ...
		 */
		status.update('listing instances of "moray" (%s)', p.new_shard);
		 ...
		 */
		status.update('provisioning Moray instances (shard %s)',
		    p.new_shard);
	...
	mod_vasync.waterfall([ function (done) {
		...
		if (server.moray_zone !== null) {
			server.status.update('zone %s (dc %s, server %s)',
			    server.moray_zone, server.dc_name, server.uuid);
		...
		ctl.create_instance({
			...
			server.status.update('zone %s (dc %s, server %s)',
			    server.moray_zone, server.dc_name, inst.uuid);

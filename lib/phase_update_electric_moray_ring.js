...
get_deployed_hash_version(ctl, zone, callback)
{
	...
	mod_vasync.waterfall([ function (done) {
		 ...
		 * SMF.
		 */
		ctl.log.info('checking for zone setup completion in "%s"',
		    zone);
		ctl.zone_exec(zone, 'svcs -Ho sta svc:/smartdc/mdata:execute',
		    function (err, res) {
			if (err) {
				done(err);
				return;
			...
			if (res.exit_status !== 0) {
				 ...
				 * XXX
				 */
				done(new VE('svcs failed'));
				return;
			}

			var out = res.stdout.trim();
			if (out !== 'ON') {
				done(new VE('not yet online'));
				return;
			}
		 ...
		 * zone.
		 */
		ctl.log.info('listing SMF instances in Electric Moray ' +
		    'zone "%s"', zone);
		ctl.zone_exec(zone,
		    'svcs -Ho fmri svc:/smartdc/application/electric-moray',
		    function (err, res) {
			if (err) {
				done(err);
				return;
		...
		mod_vasync.forEachPipeline({ inputs: Object.keys(smfi),
			...
			ctl.log.info('checking hash ring path for instance ' +
			    '"%s" in zone "%s"', i, zone);
			ctl.zone_exec(zone, 'svcprop -p ' +
			    'electric-moray/ring-location "' + i + '"',
			    function (err, res) {
				if (err) {
					next(err);
					return;
				...
				if (res.exit_status !== 0) {
					next(new VE('svcprop failed'));
					return;
				}

				smfi[i].smfi_path = res.stdout.trim();

...
zone_exec(ctx, zone_uuid, script, callback)
{
	...
	mod_vasync.waterfall([ function (done) {
		/*
		 * 1. Find the data centre that contains the zone
		 */
		ctx.ctx_sapi.getInstance(zone_uuid, function (err, res) {
			...
			log.info('found zone "%s" on server "%s" in DC "%s"',
			    zone_uuid, server, dc);
		 ...
		 */
		urc = ctx.ctx_dcs[dc].dc_clients.dcc_urclient;

		var disco = urc.discover({ timeout: 4000,
		    exclude_headnode: false, node_list: [ server ]});
		var found = false;

		disco.on('error', function (err) {
			done(new VE(err, 'ur discovery (DC %s)', dc));
		});
		disco.on('server', function (s) {
			mod_assert.strictEqual(server, s.uuid, 'uuid mismatch');

			log.info('discovered server %s (%s) in DC %s',
			    s.uuid, s.hostname, dc);
		 ...
		 */
		urc.exec({ server_uuid: server, timeout: 300 * 1000,

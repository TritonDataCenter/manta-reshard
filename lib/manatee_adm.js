...
manatee_restore_watch(opts)
{
	...
	var update_rate = function (completed, total_size) {
		if (!valid_number(completed) || !valid_number(total_size)) {
			return;
		}


		/*
		 * Keep the last 30 datapoints, which should represent about 30
		 * seconds of telemetry.  This should smooth out the rate
		...
		while (datapoints.length > 30) {
			datapoints.shift();
		}

		/*
		 * If we have at least 4 seconds worth of datapoints, begin
		 * rendering estimates.
		 */
		if (datapoints.length > 4) {
			...
			/*
			 * Render a new rate string.
			 */
			var hrd = mod_jsprim.hrtimeDiff(last.dp_when,
			    first.dp_when);
			var secs = mod_jsprim.hrtimeMillisec(hrd) / 1000;
			var delta = last.dp_completed - first.dp_completed;

			var bytes_ps = delta / secs;
			var mbytes_ps = Math.round(bytes_ps / 1024 / 1024);

			rate_str = ' ' + mbytes_ps + ' MB/sec';

			/*
			 * Estimate the time remaining.  Sometimes the
			 * estimated total size of the restore job is smaller
			 * than the actual total size -- only estimate the
			 * remaining time if that is not the case.
			 */
			var rem_bytes = total_size - completed;
			if (rem_bytes >= 1) {
				var rem_time = rem_bytes / bytes_ps;

				rate_str += '; est. ' + sec_to_hms(rem_time) +
				    ' remains';
			}
		}
	};
	...
	var fetch = function () {
		lib_http_client.http_get(opts.ip, opts.port, '/restore',
			...
			/*
			 * The "uuid" property in the restore object represents
			 * a particular restore job.  If this ID changes, it
			 * likely means the job failed to complete and another
			 * restore job has started.
			 */
			...
			update_rate(Number(r.completed), Number(r.size));

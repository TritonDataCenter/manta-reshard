	...
	var next_phase = function () {
		...
		var ctl = {
			short_shard: function () {
				var cfg = self.exe_ctx.ctx_cfg;
				var tail = cfg.region + '.' + cfg.dns_domain;
				var re = new RegExp('(.+)\\.moray\\.' +
				    tail.replace(/\./g, '\\.'));

				var m = re.exec(pr.pr_plan.shard);
				mod_assert.string(m[1], 'm[1]');

				return (m[1]);
			},
			plan: function () {
				return (mod_jsprim.deepCopy(pr.pr_plan));
			},
			...
			get_manta_app: function (callback) {
				lib_triton_access.refresh_manta_application(
				    self.exe_ctx, callback);
			},
			get_instances: function (filter, callback) {
				mod_assert.object(filter, 'filter');
				mod_assert.func(callback, 'callback');

				lib_triton_access.get_instances(self.exe_ctx,
				    filter, callback);
			},
			create_instance: function (opts, callback) {
				lib_triton_access.get_service(self.exe_ctx,
				    opts.service, function (err, res) {
					if (err) {
						callback(err);
						return;
					}

					lib_create_instance.create_instance(
					    self.exe_ctx, {
						service: res,
						shard: opts.shard,

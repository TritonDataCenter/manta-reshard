...
phase_remap_ring(ctl)
{
	...
	mod_vasync.waterfall([ function (done) {
		...
		ctl.log.info('using Electric Moray zone %s for unpack',
		    i.uuid);
		ctl.zone_exec(i.uuid, scripts['prime-workspace'].render(opts),
		    function (err, res) {
			if (err) {
				done(err);
				return;
		...
		ctl.zone_exec(i.uuid, scripts['remap-vnodes'].render(opts),
		    function (err, res) {
			if (err) {
		...
		ctl.zone_exec(i.uuid, scripts['create-archive'].render(opts),
		    function (err, res) {
			if (err) {
				done(err);
				return;

...
check_zone_setup_complete(ctl, zone, callback)
{
	...
	var fail = function (err) {
		callback(new VE(err, 'check for zone setup in "%s"', zone));
	};
	...
	ctl.log.info('checking for zone setup completion in "%s"', zone);
	ctl.zone_exec(zone, 'svcs -Ho sta "' + fmri + '"', function (err, res) {
		...
		if (out !== 'ON') {
			fail(new VE('mdata:execute is "%s", not online', out));
			return;
		}

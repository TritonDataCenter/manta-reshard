...
get_pg_conns(ctl)
{
	return (ctl.tuning_get('delete_pg_conn_count',
	    { type: 'number', min: 1, max: 128, def: 10 }));
}
	...
	mod_vasync.waterfall([ function (done) {
		...
		var again = function () {
			...
			stch.update('deleting vnode %d (#%d of %d)',
			    ...
			    vnodes.length);
			delete_one_vnode(ctl, pg_set, vnode_data.tables,
		...
		var check_again = function () {
			...
			check_one_vnode(ctl, pg_set[0], vnode_data.tables,
...
function
delete_one_vnode(ctl, pg_set, tables, vnode, status, callback)
{
	mod_assert.object(ctl, 'ctl');
	...
	/*
	 * For each table, establish a line in the status output, and a count
	 * of deleted rows.
	 */
	...
	/*
	 * In order to increase the rate of row deletion, we issue multiple
	 * delete batches in parallel.  To avoid these parallel queries
	 * overlapping, we use the modulo operation on the "_id" column when
	 * selecting records to delete.
	 *
	 * Each running instance of this function deletes all rows that
	 * have an "_id" value congruent modulo one of the N possible
	 * remainders.  For example: for 16 threads, one instance will delete
	 * all rows for this vnode where (_id % 16 === 3).
	 */
	var one_table_remainder = function (tp, _subcb) {
		if (ctl.pausing(_subcb)) {
			return;
		}

		mod_assert.string(tp.tp_table, 'tp.tp_table');
		mod_assert.number(tp.tp_remainder, 'tp.tp_remainder');
		mod_assert.object(tp.tp_pg, 'tp.tp_pg');
		mod_assert.func(_subcb, '_subcb');

		var subcb = function (err) {
			if (err) {
				err = new VE(err, 'table %s vnode %d rem %d',
				    tp.tp_table, vnode, tp.tp_remainder);
				ctl.log.error(err, 'one_table_remainder');
			}

			setImmediate(_subcb, err);
		};

		var tr = track[tp.tp_table];

		var delete_pg_conns = get_pg_conns(ctl);
		if (delete_pg_conns instanceof Error) {
			subcb(delete_pg_conns);
			return;
		}
		 ...
		 * with the correct modulus and the correct number of
		...
		status.prop('via zone', tp.tp_pg.info());
		...
		var q = [
			'DELETE FROM', tp.tp_table, 'WHERE _id IN (',
			'SELECT _id FROM', tp.tp_table, 'WHERE _vnode = $1 AND',
			'(_id % $3) = $4 LIMIT $2) AND _vnode = $1',
		].join(' ');
		var p = [ vnode, delete_batch_size, pg_set.length,
		    tp.tp_remainder ];

		tr.status.update('%-28s: deleting (%d rows)', tp.tp_table,
		    tr.count);
		ctl.log.info('checking table %s vnode %d rem %d', tp.tp_table,
		    vnode, tp.tp_remainder);
		tp.tp_pg.query({ text: q, values: p }, function (err, res) {
			if (err) {
				subcb(new VE(err, 'pg query'));
				return;
			}

			if (res.rowCount === 0) {
				ctl.log.info('no match (table %s, vnode %d, ' +
				    'rem %d)', tp.tp_table, vnode,
				    tp.tp_remainder);
				subcb();
				return;
			}
			...
			ctl.log.info('deleted %d rows for ' +
			    'table %s vnode %d rem %d', res.rowCount,
			    tp.tp_table, vnode, tp.tp_remainder);
			...
			setTimeout(one_table_remainder, delete_pause_ms, tp,
			    subcb);
		});
	};

	/*
	 * The outer loop is over each of the tables from which we need
	 * to remove this vnode.
	 */
	mod_vasync.forEachPipeline({ inputs: tables,
	    func: function (table, next) {
		if (ctl.pausing(next)) {
			return;
		}
		...
		/*
		 * While deleting for a particular table, we delete from each
		 * of the modulus phases.
		 */
		for (var i = 0; i < pg_set.length; i++) {
			tps.push({
				tp_table: table,
				tp_remainder: i,
				tp_pg: pg_set[i]
		...
		mod_vasync.forEachParallel({ inputs: tps,
		    func: one_table_remainder }, function (err) {
			if (err) {
				next(err);
				return;
			}
			...
			var tr = track[table];
			tr.status.update('%-28s: done (%d rows)', table,
			    tr.count);

...
phase_delete_data(ctl)
{
	...
	mod_vasync.waterfall([ function (done) {
		...
		delete_ghost_data(ctl, vnode_data, p.shard, p.new_shard, done);

	}, function (done) {
		delete_ghost_data(ctl, vnode_data, p.new_shard, p.shard, done);
...
function
delete_ghost_data(ctl, vnode_data, delete_from_shard, ghost_shard, callback)
{
	...
	mod_vasync.waterfall([ function (done) {
		...
		var again = function () {
			...
			delete_one_vnode(ctl, pg, vnode_data.tables,
			    vnodes[vnodes_delpos], function (err) {
				if (err) {
					done(err);
					return;
				}
...
function
delete_one_vnode(ctl, pg, tables, vnode, callback)
{
	mod_assert.object(ctl, 'ctl');
	...
	var one_table = function (table, subcb) {
		...
		pg.query({ text: q, values: p }, function (err, res) {
			if (err) {
				callback(new VE(err, 'table %s vnode %d',
				    table, vnode));
				return;
			}
